...
voice_fns = \"\"\"
  ...
  const uploadFileAndSend = async (file) => {
      ...
      try {
        ...
        console.error(err);
        setIsScanningMedia(false);
        alert("Media scan failed. Agent running?");
      }
  };
...
code = code.replace(\"bg-white border-r border-gray-200\", \"bg-[#111b21] border-r border-[#313d45]\")
...
code = code.replace(\"bg-white border-b\", \"bg-[#111b21] border-b\")
...
code = code.replace(\"bg-[#f0f2f5]\", \"bg-[#202c33]\")

with open('app/frontend/src/App.jsx', 'w', encoding='utf-8') as f:
    f.write(code)

print("Applied Dark Mode + Emojis + Voice Notes!")
